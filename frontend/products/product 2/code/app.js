...
const detailUrl = (product) => `${PRODUCT_URL}?product=${encodeURIComponent(product.slug)}`;
...
const OFFER_TIMER_KEY = 'fv_offer_timer_end_v1';
const OFFER_TIMER_MS = ((2 * 60) + 45) * 60 * 1000;
...
const initOfferCountdown = () => {
  ...
  const getEndTime = () => {
    const stored = Number(localStorage.getItem(OFFER_TIMER_KEY));
    if (Number.isFinite(stored) && stored > Date.now()) return stored;
    const next = Date.now() + OFFER_TIMER_MS;
    localStorage.setItem(OFFER_TIMER_KEY, String(next));
    return next;
  };
  ...
  const update = () => {
    ...
    if (remaining <= 0) {
      endTime = Date.now() + OFFER_TIMER_MS;
      localStorage.setItem(OFFER_TIMER_KEY, String(endTime));
      remaining = OFFER_TIMER_MS;
    }

    const totalSeconds = Math.floor(remaining / 1000);
    hrs.textContent = pad(Math.floor(totalSeconds / 3600));
    min.textContent = pad(Math.floor((totalSeconds % 3600) / 60));
